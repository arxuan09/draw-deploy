...
- 数据库里所有时间一律按 UTC 存储，由应用负责换算。不要给应用容器或 MySQL 设置本地 `TZ`,也不要在 `DATABASE_URL` 里加 `loc=` 参数;即使加了,应用启动时也会强制改回 UTC。
- 「通用」→ `ops_timezone`(运营时区,默认 `Asia/Shanghai`):后台仪表盘的「今日 / 昨日」和 30 天曲线按此时区切日,所有管理员看到同一份日报。填 IANA 时区名,如 `America/New_York`;非法值按 UTC 处理。
- 用户侧不受此项影响:每个账号有自己的时区,首次登录时按浏览器、手机号国家码、浏览器语言自动识别,设备时区变化时会提示是否切换,也可在「账户」页手动选择。页面上的时间和每日签到的换天时间都按账号时区计算;签到的时区在变更后 24 小时才生效,防止反复改时区刷签到。
